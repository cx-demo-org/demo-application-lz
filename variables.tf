...
variable "mongo_clusters" {
  ...
  validation {
    condition = alltrue([
      for k, v in var.mongo_clusters : (
        try(trimspace(v.administrator_login_password), "") != "" ||
        try(trimspace(var.mongo_cluster_admin_passwords[k]), "") != ""
      )
    ])
    error_message = "Each mongo_clusters entry must set administrator_login_password OR provide mongo_cluster_admin_passwords[cluster_key] via secret store."
  }
}

variable "mongo_cluster_admin_passwords" {
  description = "Sensitive override for Mongo vCore administrator passwords, keyed by mongo_clusters key. Prefer setting via secret stores (TF_VAR_.../GitHub Actions secrets) instead of committing in tfvars."
  type        = map(string)
  default     = {}
  sensitive   = true
}
